...
export default function ClasificacionEquipo({ id_equipo, route }) {
	...
	const cargarJornadas = async (temporada) => {
		const response = await fetch(
			`${process.env.EXPO_PUBLIC_API_URL}/partidos/jornadas?temporada=${temporada}`
		);
		if (!response.ok) {
	...
	const cargarClasificacion = async (temporada, jornada) => {
		const params = [];
		if (temporada) params.push(`anno=${temporada}`);
	...
	const cargarInicial = async () => {
		try {
			...
			const [equiposMap, temporadasLista] = await Promise.all([
				cargarEquipos(),
				cargarTemporadas(),
			]);

			const temporadaBase = temporadasLista[0] ?? null;
			let jornadasData = { jornadas: [], jornadaActual: null };
			if (temporadaBase) {
				jornadasData = await cargarJornadas(temporadaBase);
			}
			const jornadaBase = jornadasData.jornadaActual ?? jornadasData.jornadas[jornadasData.jornadas.length - 1] ?? null;

			const rows = temporadaBase && jornadaBase
				? await cargarClasificacion(temporadaBase, jornadaBase)
				: [];

			setEquiposById(equiposMap);
			setTemporadas(temporadasLista);
	...
	const seleccionarTemporada = async (temporada) => {
		if (temporada === temporadaSeleccionada) return;
		try {
			setCargando(true);
			setError('');

			const jornadasData = await cargarJornadas(temporada);
			const jornadaBase = jornadasData.jornadaActual ?? jornadasData.jornadas[jornadasData.jornadas.length - 1] ?? null;
			const rows = jornadaBase ? await cargarClasificacion(temporada, jornadaBase) : [];
	...
	const seleccionarJornada = async (jornada) => {
		if (jornada === jornadaSeleccionada || !temporadaSeleccionada) return;
		try {
			setCargando(true);
			setError('');
			const rows = await cargarClasificacion(temporadaSeleccionada, jornada);
	...
	const renderFila = (row, idx) => {
		...
		return (
			<TouchableOpacity
				...
				style={[styles.row, idx % 2 === 1 && styles.rowAlt, id === Number(equipoId) && styles.rowHighlight]}
				...
				<ScrollView horizontal showsHorizontalScrollIndicator={false}>
					<View style={styles.selectorRow}>
						{temporadas.map((temp) => (
							<TouchableOpacity
								key={`temp-${temp}`}
								style={[styles.chip, temp === temporadaSeleccionada && styles.chipActive]}
								onPress={() => seleccionarTemporada(temp)}
								activeOpacity={0.85}
							>
								<Text style={[styles.chipText, temp === temporadaSeleccionada && styles.chipTextActive]}>
									{temp}
								</Text>
							</TouchableOpacity>
						))}
