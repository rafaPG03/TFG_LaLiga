...
const ChartShell = ({ width, children }) => (
    <View style={[styles.lineChartBox, { width, height: LINE_CHART_HEIGHT }]}>
        <Svg width={width} height={LINE_CHART_HEIGHT} viewBox={`0 0 ${width} ${LINE_CHART_HEIGHT}`}>
            {children}
        </Svg>
    </View>
);
    ...
    return (
        <G opacity={0.95}>
            {logo ? (
                ...
                <Circle cx={x} cy={centerY} r={size / 2} fill="#1f6fa7" />
            )}
        </G>
    );
};
...
export default function StatsJugador({ id_jugador, route }) {
    ...
    const fetchPartidos = async (jugador, temporada) => {
        const queryTemporada = temporada ? `?temporada=${encodeURIComponent(temporada)}` : '';
        const response = await fetch(
            `${process.env.EXPO_PUBLIC_API_URL}/jugadores/partidos/${jugador}${queryTemporada}`
        );
    ...
    const cargarInicial = async () => {
        try {
            ...
            const partidosData = temporadaBase ? await fetchPartidos(jugadorId, temporadaBase) : [];
    ...
    const seleccionarTemporada = async (temporada) => {
        ...
        try {
            ...
            const [ratingsData, partidosData] = await Promise.all([
                fetchRatings(jugadorId, temporada),
                fetchPartidos(jugadorId, temporada),
            ]);
    ...
    const lineData = useMemo(
        () =>
            partidos
                .filter((partido) => toNumber(partido.nota) > 1 && toNumber(partido.jornada) > 0)
                .map((partido) => {
                    const jugadorEsLocal = Number(partido.id_equipo_jugador) === Number(partido.id_local);
                    return {
                        x: toNumber(partido.jornada),
                        y: toNumber(partido.nota),
                        rival: jugadorEsLocal ? partido.equipo_visitante : partido.equipo_local,
                        rival_logo: jugadorEsLocal ? partido.logo_visitante : partido.logo_local,
                    };
                })
                .sort((a, b) => a.x - b.x),
        [partidos]
    ...
    const lineAxisStyle = useMemo(
        () => ({
            axis: { stroke: '#d9e5f0' },
            axisLabel: { fill: '#0f2743', fontSize: 11, padding: 34 },
            tickLabels: { fill: '#5a7189', fontSize: 9 },
            grid: { stroke: '#d9e5f0', strokeDasharray: '4,4' },
        }),
        []
    );
    ...
    if (cargando) {
        return (
            <View style={styles.estadoPantalla}>
                ...
                <Text style={styles.estadoTexto}>Cargando estadísticas...</Text>
            </View>
        );
    }
            ...
            <ScrollView contentContainerStyle={styles.containerContent}>
                ...
                {temporadaSeleccionada ? (
                    <View style={styles.card}>
                        <Text style={styles.cardTitle}>Rating por partido</Text>
                        <Text style={styles.cardSubtitle}>
                            Evolución del jugador por jornada. Solo se muestran notas superiores a 1.
                        </Text>
                        {lineData.length > 0 ? (
                            <ChartShell width={lineChartWidth}>
                                <VictoryChart
                                    standalone={false}
                                    ...
                                    domain={{ y: [1, 10] }}
                                >
                                    <VictoryAxis label="Jornada" style={lineAxisStyle} />
                                    <VictoryAxis
                                        dependentAxis
                                        label="Rating"
                                    ...
                                    <VictoryLine
                                        ...
                                        style={{ data: { stroke: '#e20613', strokeWidth: 3 } }}
                                    />
                                    <VictoryScatter
                                        data={lineData}
                                        size={4}
                            ...
                            <Text style={styles.lineEmptyText}>
                                No hay ratings válidos para esta temporada.
                            </Text>
                        )}
                    </View>
                ) : null}
