...
export default function DetalleEquipoScreen({navigation, route}) {
   const idEquipo = route?.params?.idEquipo ?? route?.params?.id ?? route?.params?.id_equipo;
   ...
   const cargarDatosEquipo = async () => {
      try {
         ...
         const response = await fetch(`${process.env.EXPO_PUBLIC_API_URL}/equipos/${idEquipo}`);
   ...
   return (
      ...
      >
         ...
         <View style={styles.equipoCard}>
            ...
            {idEquipo ? <FavoritoButton id={idEquipo} tipo="equipo" style={styles.favoritoDetalle} /> : null}
         </View>
