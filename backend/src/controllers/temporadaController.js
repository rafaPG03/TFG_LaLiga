...
const RANKING_METRICAS = {
    nota_media: { label: 'Nota media', grupo: 'General', orden: 'DESC', precision: 2, per90: false, expresion: 'COALESCE(h.nota_media, 0)::numeric' },
    partidos: { label: 'Partidos', grupo: 'General', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.partidos, 0)::numeric' },
    minutos: { label: 'Minutos', grupo: 'General', orden: 'DESC', precision: 0, per90: false, expresion: 'COALESCE(h.minutos, 0)::numeric' },
    titular: { label: 'Titularidades', grupo: 'General', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.titular, 0)::numeric' },
    paradas: { label: 'Paradas', grupo: 'Porteria', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.paradas, 0)::numeric' },
    goles_concedidos: { label: 'Goles concedidos', grupo: 'Porteria', orden: 'ASC', precision: 0, per90: true, expresion: 'COALESCE(h.goles_concedidos, 0)::numeric' },
    penaltis_parados: { label: 'Penaltis parados', grupo: 'Porteria', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.penaltis_parados, 0)::numeric' },
    entradas: { label: 'Entradas', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.entradas, 0)::numeric' },
    bloqueos: { label: 'Bloqueos', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.bloqueos, 0)::numeric' },
    intercepciones: { label: 'Intercepciones', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.intercepciones, 0)::numeric' },
    duelos_ganados: { label: 'Duelos ganados', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.duelos_ganados, 0)::numeric' },
    duelos_totales: { label: 'Duelos totales', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.duelos_totales, 0)::numeric' },
    faltas_cometidas: { label: 'Faltas cometidas', grupo: 'Defensa', orden: 'ASC', precision: 0, per90: true, expresion: 'COALESCE(h.faltas_cometidas, 0)::numeric' },
    regateado: { label: 'Regateado', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.regateado, 0)::numeric' },
    amarillas: { label: 'Amarillas', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.amarillas, 0)::numeric' },
    rojas: { label: 'Rojas', grupo: 'Defensa', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.rojas, 0)::numeric' },
    asistencias: { label: 'Asistencias', grupo: 'Creacion', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.asistencias, 0)::numeric' },
    pases_totales: { label: 'Pases totales', grupo: 'Creacion', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.pases_totales, 0)::numeric' },
    pases_clave: { label: 'Pases clave', grupo: 'Creacion', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.pases_clave, 0)::numeric' },
    precision_pases: { label: 'Precision pases', grupo: 'Creacion', orden: 'DESC', precision: 0, per90: false, expresion: 'COALESCE(h.precision_pases, 0)::numeric' },
    regates_intentados: { label: 'Regates intentados', grupo: 'Creacion', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.regates_intentados, 0)::numeric' },
    regates_exito: { label: 'Regates exitosos', grupo: 'Creacion', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.regates_exito, 0)::numeric' },
    goles: { label: 'Goles', grupo: 'Ataque', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.goles, 0)::numeric' },
    tiros_totales: { label: 'Tiros totales', grupo: 'Ataque', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.tiros_totales, 0)::numeric' },
    tiros_a_puerta: { label: 'Tiros a puerta', grupo: 'Ataque', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.tiros_a_puerta, 0)::numeric' },
    faltas_sufridas: { label: 'Faltas sufridas', grupo: 'Ataque', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.faltas_sufridas, 0)::numeric' },
    penaltis_marcados: { label: 'Penaltis marcados', grupo: 'Ataque', orden: 'DESC', precision: 0, per90: true, expresion: 'COALESCE(h.penaltis_marcados, 0)::numeric' },
};
...
const getRankingsTemporada = async (req, res) => {
    try {
        ...
        const posicionValores = posicion === 'TODAS' ? null : RANKING_POSICIONES[posicion];

        if (posicion !== 'TODAS' && !posicionValores) {
            return res.status(400).json({ error: 'Posicion de ranking invalida' });
        }

        const expresionValor = modo === 'por90' && metric.per90
...
const getGraficosTemporada = async (req, res) => {
    try {
        ...
        res.json({
            ...
            partidos_totales_liga: toIntOrNull(partidosRes.rows[0]?.partidos_totales_liga) ?? 0,
            equipos,
            jugadores,
        });
    } catch (error) {
...
const getPartidosTemporada = async (req, res) => {
    try {
        ...
        if (!Number.isInteger(temporadaBusqueda)) {
            return res.status(400).json({ error: 'La temporada debe ser un numero entero' });
        }

        if (jornada && !Number.isInteger(jornadaBusqueda)) {
            return res.status(400).json({ error: 'La jornada debe ser un numero entero' });
        }
